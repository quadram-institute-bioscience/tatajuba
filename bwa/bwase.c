...
void bwa_print_sam1 (const bntseq_t *bns, bwa_seq_t *p, const bwa_seq_t *mate, int mode, int max_top2)
{
  ...
  if (p->type != BWA_TYPE_NO_MATCH || (mate && mate->type != BWA_TYPE_NO_MATCH)) {
    ...
    err_printf("%s\t%d\t%s\t", p->name, flag, bns->anns[seqid].name);// query name, flag, ref sequence name 
    err_printf("%d\t%d\t", (int)(p->pos - bns->anns[seqid].offset + 1), p->mapQ); // 1-based leftmost position, mapping quality
...
{ // leo: prefix, sai (fn_sa), fastq (fn_fa); needs prefix since reads indices several times in different ways (cal_pac_pos())
  ...
  for (i = 0; i < n_dnaseq; i++) {
    ...
    if (p->n_aln > m_aln) {
      m_aln = p->n_aln;
      aln = (bwt_aln1_t*) realloc ((bwt_aln1_t*) aln, sizeof (bwt_aln1_t) * m_aln);
    }
    for (j = 0; j < p->n_aln; j++) *(aln + j) = *(p->aln + j); // leo: I don't know if aln is modified below
    bwa_aln2seq_core (p->n_aln, aln, p, 1, n_occ);
  }

  fprintf(stderr, "[bwa_aln_from_vector] convert to sequence coordinate... ");
