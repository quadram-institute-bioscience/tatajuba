...
bwa_seq_t *bwa_read_seq (bwa_seqio_t *bs, int n_needed, int *n, int mode, int trim_qual)
{
  ...
  while ((l = kseq_read(seq)) >= 0) {
    ...
    if (is_64 && seq->qual.l) for (i = 0; i < seq->qual.l; ++i) seq->qual.s[i] -= 31;
    if (seq->seq.l <= l_bc) continue; // sequence length equals or smaller than the barcode length
    p = &seqs[n_seqs++];
    if (l_bc) { // then trim barcode
      for (i = 0; i < l_bc; ++i) p->bc[i] = (seq->qual.l && seq->qual.s[i]-33 < BARCODE_LOW_QUAL)? tolower(seq->seq.s[i]) : toupper(seq->seq.s[i]);
      p->bc[i] = 0;
      for (; i < seq->seq.l; ++i) seq->seq.s[i - l_bc] = seq->seq.s[i];
      seq->seq.l -= l_bc; seq->seq.s[seq->seq.l] = 0;
      if (seq->qual.l) {
        for (i = l_bc; i < seq->qual.l; ++i) seq->qual.s[i - l_bc] = seq->qual.s[i];
...
bwa_seq_t *bwa_read_seq_from_vector (char **seqname, char **dnaseq, char **qual, size_t *seq_len, int n_dnaseq, int trim_qual) // BIOMCMC
{
  bwa_seq_t *seqs, *p;
  int n_seqs = 0, j, i;
  long n_trimmed = 0, n_tot = 0;
